...
A NULL `techniques` column is `null` in both fields. **JS clients should read `<field>_bitmask` and parse it with `BigInt(...)`**, and test bits with `(mask >> BigInt(id)) & 1n`. Do not read the numeric field. For the two solver routes, the API takes the solver's own `techniques_bitmask` when present and otherwise falls back to `String(techniques)`, which is lossy for older solver deployments.

**Requests.** Bitmask inputs accept a JSON number or a decimal string, parsed with `BigInt`. Send a string for anything above 2^53. This covers `techniques` in `boardCreateSchema` / `boardUpdateSchema` / `dailyCreateSchema` / `dailyUpdateSchema` / `gameStartSchema`, `techniques_bitfield` in the example schemas (which must be at least 1), and the `/boards` query params `techniques` and `technique_bit`. Negative, fractional, exponent, hex, empty-string or out-of-range (> 2^63 − 1) values are rejected. Bodies fail with the usual zod-validator 400. The `/boards` query params fail with 400 `errorResponse("Invalid techniques: …")`. A body `null` still means 0.
